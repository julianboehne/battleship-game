package de.htwg.se.battleship.model
//TODO
import de.htwg.se.battleship.model.gridImpl.GridForShots
import org.scalatest.wordspec.AnyWordSpec
import org.scalatest.matchers.should.Matchers.*

class GridForShotsSpec extends AnyWordSpec {

  /*val test = GridForShots(3,Vector("A1","B1","C1","A2","B2","C2","A3","B3","C3"))
    "GridForShots" should {
      "have a field function" in {
        test.field(1,1) should be("rofl")
      }
      }*/

/*  val nextline: String = sys.props("line.separator")
  "A GridForShots" should {
    "correctly implement the vertical function" in {
      val grid = Vector("A1","B1","C1","A2","B2","C2","A3","B3","C3")
      val gridForShots = GridForShots(3, grid)
      gridForShots.vertical(3, 3, 0) should be("|A1|B1|C1|A2|B2|C2|A3|B3|C3|"+nextline)
    }

    "correctly implement the field function" in {
      val grid = Vector("A1","B1","C1","A2","B2","C2","A3","B3","C3")
      val gridForShots = GridForShots(3, grid)
      println( gridForShots.field(4, 3) )
      gridForShots.field(4, 3) should be("")
    }*/
  }

