  ...
  private val scn = scenario("RecordedSimulation")
    .exec(
      http("request_0")
        ...
        .headers(headers_0)
        .body(RawFileBody("resources/0000_request.txt"))
        .check(bodyBytes.is(RawFileBody("resurces/0000_response.txt"))),
      pause(1),
      http("request_1")
        .post("/persistence/load")
        .headers(headers_1)
        .body(RawFileBody("resources/0001_request.json"))
        .check(bodyBytes.is(RawFileBody("resources/0001_response.json"))),
      pause(1),
      http("request_2")
        .post("/persistence/save")
        .headers(headers_2)
        .body(RawFileBody("resources/0002_request.txt"))
        .check(bodyBytes.is(RawFileBody("resources/0002_response.txt"))),
      pause(1),
      http("request_3")
        .post("/persistence/load")
        .headers(headers_3)
        .body(RawFileBody("resources/0003_request.json"))
        .check(bodyBytes.is(RawFileBody("resources/0003_response.json")))
    )
